...
export default function HomeWritingPreview({ writings }: { writings: HomeWriting[] }) {
  ...
  return <section id="writing" className="portfolio-deferred w-full max-w-2xl mx-auto px-5 sm:px-8 py-12 scroll-mt-20" aria-labelledby="home-writing-title">
    <h2 id="home-writing-title" className="mb-8 text-sm font-normal leading-relaxed tracking-[0.01em] text-foreground">Notes from my desk</h2>
    <div className="article-index">
      {writings.map((writing, index) => <Link key={writing.id} href={`/writing/${encodeURIComponent(slugify(writing.title) || writing.slug)}`} className="article-index-link group">
        <div className="article-index-row">
          <h3>{writing.title}{index === 0 && <small className="home-new-marker">New</small>}</h3>
