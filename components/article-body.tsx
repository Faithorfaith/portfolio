...
export default function ArticleBody({ blocks, typeface = 'sans' }: { blocks: ArticleBlock[]; typeface?: 'sans' | 'handwritten' }) {
  return <div className={`w-full max-w-[600px] text-sm ${typeface === 'handwritten' ? 'writing-handwritten-article' : ''}`}>
    {blocks.map((block) => {
      ...
      return <SafeHtml key={block.id} as={block.type === 'quote' ? 'blockquote' : 'div'} html={block.content} className={`article-motion-block article-prose ${block.type === 'quote' ? 'pl-5 border-l border-foreground/20 my-8' : 'mb-6'}`} />
    })}
  </div>
}
