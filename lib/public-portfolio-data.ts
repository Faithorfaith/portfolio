...
export const getHomepagePortfolioData = unstable_cache(async (): Promise<HomepagePortfolioData> => {
  ...
  const [profileResult, caseStudiesResult, projectsResult, worksResult, writingsResult, workflowResult] = await Promise.all([
    supabase.from('profiles').select('id, username, full_name, bio, avatar_url, hero_image_1, hero_image_2, hero_image_3, gallery_images, bio_references, supporting_statement, availability_status, contact_email, train_music_url, train_video_1, train_video_2, train_video_3, train_video_4').limit(1).maybeSingle(),
