  ...
  return (
    <main className="flex flex-col w-full h-screen bg-background">
      ...
      <div className="flex-1 overflow-hidden md:pl-32">
        {activeTab === 'home' && <div ref={homeRef} className="h-full overflow-y-auto pb-20 md:pb-0"><ProfileSection profile={profile} caseStudies={caseStudies} /><ProjectsSection projects={projects} /></div>}
        {activeTab === 'ui-shots' && <div ref={playgroundRef} className="h-full overflow-y-auto pb-20 md:pb-0"><WorksGallery onSubPageChange={setIsSubPage} /></div>}
        {activeTab === 'writing' && <div ref={writingRef} className="h-full overflow-y-auto pb-20 md:pb-0"><WritingSection onSubPageChange={setIsSubPage} /></div>}
      </div>
    </main>
  )
}
